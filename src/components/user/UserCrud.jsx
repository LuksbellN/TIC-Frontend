...
export default class UserCrud extends Component {
    ...
    async save() {
        ...
        await axios[method](url, user)
          .then(async resp => {
            const list = await this.getUpdatedList(resp.data.data, true);
            this.setState({ user: initialState.user, list });
          })
      }
      

    async getUpdatedList(user, add = true) {
        const list = await this.state.list.filter(u => u.id !== user.id);
        ...
        return (
          <div className="form">
            <div className="row">
              ...
              <div className="col-12 col-md-3">
                <div className="form-group">
                  <label htmlFor="senha">Departamento</label>
                  <select
                    className="form-control"
    ...
    remove(user) {
        axios.delete(`${baseUrl}/usuario/${user.id}`).then(resp => {
